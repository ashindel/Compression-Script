...
function Invoke-DBCompressScript {
    ...
    if ( -Not (Get-ChildItem -Path $Path -Directory | Where-Object {$_.PSIsContainer -eq $true -and $_.Name -like $dbDumpFolderName} )) {
        ...
        # Add all files in dbdump folder to master list
        Get-ChildItem -Path $dbDumpFullPath -File | Format-Table -HideTableHeaders FullName -AutoSize | out-file $MasterList   
        ...
        $allChildFiles = Get-ChildItem -Path $dbDumpFullPath | Where-Object {$_.extension -in $SQLFileExtension} ## All .sql files in dbdump folder
        
        $allChildFiles | Format-Table @{N="$SQLFileExtension files in $DBDumpFolderPath folder";E={$_.name}}, CreationTime, @{N='File Size';E={Get-FriendlySize -Bytes $_.Length}} -AutoSize |
        ...
        Get-ChildItem -Path $ArchivedFullPath -File -Recurse | Select-Object @{N="Zipped files from $DBDumpFolderPath folder";E={$_.name}}, @{N='File Size';E={Get-FriendlySize -Bytes $_.Length}} |
    ...
    if ($ReverseCreatedItemsParam -eq 1) {
        ...
        return "The Invoke-DBCompressScript script has terminated" ## set function name to variable or find how to grab it
    }
}
# Run Invoke-DBCompressScript. Specify path to compress and the folder name of files to be archived (comspressed)
Invoke-DBCompressScript -Path C:\users\ajs573\Documents -ArchivedFolderPath "\archived"
